...
primary_raw(Parser *p)
{
    ...
    { // primary '[' arguments ']'
        ...
        )
        {
            ...
            int _end_col_offset = _token->end_col_offset;
            UNUSED(_end_col_offset); // Only used by EXTRA macro
            _res = _Py_Subscript ( a , NULL , NULL , Load , EXTRA );
            if (_res == NULL && PyErr_Occurred()) {
                p->error_indicator = 1;
                D(p->level--);
                return NULL;
...
t_primary_raw(Parser *p)
{
    ...
    { // t_primary '[' kwargs ']'
        ...
        D(fprintf(stderr, "%*c> t_primary[%d-%d]: %s\n", p->level, ' ', _mark, p->mark, "t_primary '[' kwargs ']'"));
        Token * _literal;
        Token * _literal_1;
        expr_ty a;
        asdl_seq* c;
        ...
        )
        {
            ...
            int _end_col_offset = _token->end_col_offset;
            UNUSED(_end_col_offset); // Only used by EXTRA macro
            _res = _Py_Subscript ( a , NULL , _PyPegen_seq_delete_starred_exprs ( p , c ) , Load , EXTRA );
            if (_res == NULL && PyErr_Occurred()) {
                p->error_indicator = 1;
                D(p->level--);
                return NULL;
        ...
        p->mark = _mark;
        D(fprintf(stderr, "%*c%s t_primary[%d-%d]: %s failed!\n", p->level, ' ',
                  p->error_indicator ? "ERROR!" : "-", _mark, p->mark, "t_primary '[' kwargs ']'"));
    }
    { // t_primary '[' slices ']' &t_lookahead
        if (p->error_indicator) {
            D(p->level--);
